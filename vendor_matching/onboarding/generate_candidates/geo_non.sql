...
(
    SELECT
        ...
        bqcarto.h3.LONGLAT_ASH3(city_lng, city_lat, 5) AS h3_idx --add h3 index based on lat/lng of city, h3 resolution=5 (hexagons with an average edge length of approximately 2.4 km)
    FROM
...
(
    ...
    WHERE
        (lat+lng) IS NULL --no geo data for right side
)
...
(
--Join left and right data based on city h3 index
    SELECT DISTINCT
        l.country_iso,
        left_row_id,
        left_name,
...
--Insert into candidates geo_non table based on name similarity or phone number match
