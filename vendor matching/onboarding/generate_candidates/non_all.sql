...
SELECT
    ...
    bqcarto.h3.LONGLAT_ASH3(city_lng, city_lat, 5) AS h3_idx --geo data based on city lat/lng h3 index
FROM
    `dh-global-sales-data.leadgen_sf_match_vertex_raw.left_processed_PO_FI`
WHERE
    (lat+lng) IS NULL AND --no geo data for left side
...
--Extract DH data (right side), all of it without geo conditions
CREATE TEMP TABLE
    right_table
AS
(
...
(
    SELECT
        ...
        bqcarto.h3.KRING(bqcarto.h3.LONGLAT_ASH3(city_lng, city_lat,5), 2) AS h3_idx --geo data based on city lat/lng h3 index ring
    FROM
        `dh-global-sales-data.leadgen_sf_match_vertex_raw.right_processed_PO_FI` AS r
)
...
--Match left and right based on city level geo matches
...
--Insert into candidates non_all table based on name similarity or phone number match
INSERT INTO `dh-global-sales-data-dev.leadgen_sf_match_vertex_raw.candidates_PO_FI_non_all`
(
WITH tsr AS
(
    SELECT
