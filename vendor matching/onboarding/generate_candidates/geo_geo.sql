...
--Extract potential lead data corresponding to relevant lead source (left side)
CREATE TEMP TABLE
    left_table
AS
(
SELECT
    ...
    bqcarto.h3.LONGLAT_ASH3(lng, lat, 8) AS h3_idx, --add h3 index based on lat/lng of address, h3 resolution=8 (hexagons with an average edge length of approximately 460 meters)
    bqcarto.h3.LONGLAT_ASH3(city_lng, city_lat, 5) AS city_h3_idx --add h3 index based on lat/lng of city, h3 resolution=5 (hexagons with an average edge length of approximately 2.4 km)
FROM
    `dh-global-sales-data.leadgen_sf_match_vertex_raw.left_processed_PO_FI`
WHERE
    (lat+lng) IS NOT NULL AND --geo data has to be present for leads
    mod(abs(FARM_FINGERPRINT(row_id)), 1) = 0 --this splits the data into shards, in this case only 1 shard so always true
    AND `dh-global-sales-data.achilles.row_id_to_lead_source`(row_id) = 'facebook' --get only rows with desired lead source
)
;

--Extract DH data (right side)
CREATE TEMP TABLE
    right_table
AS
(
...
(
    SELECT
        ...
        bqcarto.h3.KRING(bqcarto.h3.LONGLAT_ASH3(lng, lat,8), 2) AS h3_idx, --generates a list of H3 indexes representing a ring of hexagons around a given point
    FROM
        `dh-global-sales-data.leadgen_sf_match_vertex_raw.right_processed_PO_FI`
    WHERE
        (lat+lng) IS NOT NULL --geo data has to be present
)
...
FROM
    rt,
    UNNEST(rt.h3_idx) AS hex --unnest H3 index list for right data
)
;

--Same process for right data but now using city geo data
CREATE TEMP TABLE
    right_table_city
AS
(
...
(
    SELECT
        ...
        lng AS right_lng,
        registration_number AS right_registration_number,
        bqcarto.h3.KRING(bqcarto.h3.LONGLAT_ASH3(city_lng, city_lat,5), 2) AS city_h3_idx,
    FROM
        `dh-global-sales-data.leadgen_sf_match_vertex_raw.right_processed_PO_FI`
    WHERE
        (lat+lng) IS NOT NULL
...
FROM
    rt,
    UNNEST(rt.city_h3_idx) AS hex
)
;

--Match left and right based on geo matches
CREATE TEMP TABLE geofilter AS (
--Join left and right data based on address h3 index 
    SELECT 
        l.country_iso,
        left_row_id,
        left_name,
...
--Join left and right data based on city h3 index and phone number match
    SELECT 
        l.country_iso,
        left_row_id,
        left_name,
    ...
    FROM
        left_table AS l
    INNER JOIN
        right_table_city AS r
    ON
        l.city_h3_idx = r.city_h3_idx AND
        l.left_phone_number = r.right_phone_number
)
;

--Insert into candidates geo_geo table based on name similarity or phone number match
INSERT INTO `dh-global-sales-data-dev.leadgen_sf_match_vertex_raw.candidates_PO_FI_geo_geo`
(
WITH tsr AS
(
    SELECT
        *,
        ST_DISTANCE(ST_GEOGPOINT(left_lng, left_lat), ST_GEOGPOINT(right_lng, right_lat))/1000.0 AS haversine,
